...
- global.json — the .NET SDK pin, second half of the analyzer-stability guarantee added with the AnalysisLevel pin in a97e930 (Directory.Build.props sets AnalysisMode=All + TreatWarningsAsErrors, so an unpinned SDK can turn new CA rules into build errors with no commit of ours). The SDK discovers global.json ONLY by walking UP from the build directory to the first one it finds, so the repo root is the only location that covers the whole tree; under eng/ the pin silently applies to nothing. MEASURED 2026-08-04 with an intentionally unsatisfiable pin (version 99.0.100, rollForward=disable): placed under eng/, `dotnet --version` in a sibling dir ignored it and printed 10.0.108; placed at the root of the walk, the same command failed to resolve an SDK. The path IS the contract, exactly as for ruff.toml below. (g-dnfleet, 2026-08-04 — location-mandated, pending owner confirmation)
