...
signalwire.relay.event.Event: .NET-only RAW DISPATCH envelope (src/SignalWire/Relay/Event.cs) carrying event_type/timestamp/params off the wire frame, consumed by Client/Call/Message/Action to route a frame before any typed wrapper is chosen. It is NOT the reference's RelayEvent under another name: the port ALSO ships signalwire.relay.event.RelayEvent (Events.cs:19) with the reference's exact member set (__init__/event_type/params/call_id/timestamp/from_payload) plus the 23 typed subclasses, and Call.on/wait_for*/wait_for now take and return THAT class, matching the reference. The reference has no separate raw-envelope type - Python routes the raw payload dict directly - so Event has no reference twin. NOTE: the prior rationale claimed "Python ships RelayEvent under signalwire.relay.constants"; the reference ships RelayEvent in signalwire/relay/event.py and constants.py declares no classes at all - corrected 2026-07-28.
