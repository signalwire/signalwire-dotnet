...
signalwire.relay.call.Call.live_transcribe: .NET takes the required `action` dict as an explicit named parameter (matches Python's `action` param + wire params.action wrap) plus a single Dictionary<string,object> `extra` to forward additional protocol args; Python explodes those into named keyword arguments
...
signalwire.rest._base.SignalWireRestTransportError.__init__: .NET SignalWireRestTransportError ctor (plan 1.3b) takes (message, url, method, inner_exception) — a .NET Exception carries its human-readable text as a ctor `message` positional and the underlying transport exception as `inner_exception` (the standard .NET Exception(message, innerException) idiom, the C# equivalent of Python's `raise ... from exc`), with status_code/body implicitly 0/empty (this port's no-status sentinel for "no HTTP response was ever received") rather than explicit params; Python's ctor takes (body, url, method) with no separate message/cause param (it formats its own message and has no `from`-exception slot). Same transport-error envelope (status=None/0, url, method), message-vs-no-message and explicit-vs-implicit body/inner-exception idiom.
