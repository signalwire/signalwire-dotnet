...
signalwire.relay.client.RelayError.__init__: .NET RelayError ctor takes code as a string (the RELAY error code string) and message typed via the exception base; Python types code as int and message as string — same error envelope, string-vs-int code idiom (the RELAY wire code is carried verbatim either way)
