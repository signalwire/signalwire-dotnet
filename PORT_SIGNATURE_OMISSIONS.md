...
signalwire.core.mixins.auth_mixin.AuthMixin.get_basic_auth_credentials: impossible: a C# method's STATIC return type cannot depend on an argument's VALUE. The reference is one method whose runtime shape is selected by a flag - get_basic_auth_credentials(include_source: bool = False) -> tuple[str,str] | tuple[str,str,str] (signalwire/core/mixins/auth_mixin.py:47) - so a 2-tuple for include_source=False and a 3-tuple for True. C# has no literal types (TypeScript closes this with `includeSource: true` overloads) and no anonymous union, so a `GetBasicAuthCredentials(bool)` overload would have to return the 3-tuple even when passed false, inventing a shape the reference never returns. The port instead ships the two arms as separately-named methods - GetBasicAuthCredentials() -> (User,Password) and GetBasicAuthCredentialsWithSource() -> (User,Password,Source), src/SignalWire/SWML/Service.cs:284,293 - so BOTH capabilities are reachable and neither shape is fabricated. Same shape in go/rust/java/php/cpp; only perl (dynamic return) and TypeScript (literal-typed overloads) can express the reference form. NOTE: the prior rationale claimed an overload returning a union; no such overload exists and C# cannot declare one - corrected 2026-07-28.
signalwire.core.mixins.prompt_mixin.PromptMixin.define_contexts: the set-contexts arm is unreachable from AgentBase (a missing capability, not an idiom difference). The reference define_contexts(contexts=None) has TWO arms (signalwire/core/mixins/prompt_mixin.py:131): passing a dict/ContextBuilder SETS the contexts and returns the agent; passing nothing returns the lazily-created ContextBuilder. .NET's AgentBase.DefineContexts() (src/SignalWire/Agent/AgentBase.cs:1407) takes NO parameter and only ever returns the builder, so the set-from-dict arm is UNREACHABLE from AgentBase. Closing it needs a dict->ContextBuilder import path, which ContextBuilder does not have (no FromDict / dict ctor). Not an overload limitation - there is no second overload. NOTE: the prior rationale claimed a union arising from overloads; the C# side has a single zero-arg method - corrected 2026-07-28. PromptManager.DefineContexts(object) DOES implement the set arm and now compares (its omission was retired).
...
signalwire.core.swml_service.SWMLService.get_basic_auth_credentials: impossible: a C# method's STATIC return type cannot depend on an argument's VALUE. The reference is one method whose runtime shape is selected by a flag - get_basic_auth_credentials(include_source: bool = False) -> tuple[str,str] | tuple[str,str,str] (signalwire/core/mixins/auth_mixin.py:47) - so a 2-tuple for include_source=False and a 3-tuple for True. C# has no literal types (TypeScript closes this with `includeSource: true` overloads) and no anonymous union, so a `GetBasicAuthCredentials(bool)` overload would have to return the 3-tuple even when passed false, inventing a shape the reference never returns. The port instead ships the two arms as separately-named methods - GetBasicAuthCredentials() -> (User,Password) and GetBasicAuthCredentialsWithSource() -> (User,Password,Source), src/SignalWire/SWML/Service.cs:284,293 - so BOTH capabilities are reachable and neither shape is fabricated. Same shape in go/rust/java/php/cpp; only perl (dynamic return) and TypeScript (literal-typed overloads) can express the reference form. NOTE: the prior rationale claimed an overload returning a union; no such overload exists and C# cannot declare one - corrected 2026-07-28.
