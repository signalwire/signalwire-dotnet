    ...
    internal class Program
    {
        ...
        private static void CallingAPI_OnCallReceiveCreated(CallingAPI api, Call call, CallEventParams.ReceiveParams receiveParams)
        {
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    call.OnPlayUpdate += OnCallPlayUpdate;
                    call.Play(
                        "im_a_little_teapot",
                        new List<CallPlay>
                        {
                            new CallPlay()
                            {
                                Type = CallPlay.PlayType.tts,
                                Parameters = JObject.FromObject(new CallPlay.TTSParams()
                                {
                ...
                {
                    Logger.LogError(exc, "CallPlay failed");
                    sCompleted.Set();
                    return;
                }
            });
        ...
        private static void OnCallPlayUpdate(CallingAPI api, Call call, CallEventParams.PlayParams playParams)
        {
            Logger.LogInformation("OnCallPlayUpdate: {0}, {1} for {2}, {3}", call.CallID, call.State, playParams.ControlID, playParams.State);
            if (playParams.State == CallEventParams.PlayParams.PlayState.finished)
            {
                Logger.LogInformation("OnCallPlayUpdate completed successfully");
                sSuccessful = true;
                sCompleted.Set();
            }
        }
