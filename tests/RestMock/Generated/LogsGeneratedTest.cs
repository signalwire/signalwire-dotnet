...
public class LogsGeneratedTest : CoverageBase
{
    ...
    public async Task LogsConferences_List_Success()
    {
        ...
        var tree = NewTree();
        var body = await tree.Logs.Conferences.ListAsync();
        Assert.NotNull(body);
        var j = Fixture.Harness.Journal.Last();
        Assert.Equal("GET", j.Method);
        Assert.Equal("logs.list_conferences", j.MatchedRoute);
    }

    [Fact]
    public async Task LogsConferences_List_Error()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var status = await AssertErrorAsync("logs.list_conferences", 500,
            () => tree.Logs.Conferences.ListAsync());
