...
/// <summary>Generated full-mock REST wire tests for the 'messages' namespace.</summary>
...
public class MessagesGeneratedTest : CoverageBase
{
    ...
    public async Task Messages_Create_Success()
    {
        ...
        var tree = NewTree();
        var body = await tree.Messages.CreateAsync("x", "x");
        Assert.NotNull(body);
        var j = Fixture.Harness.Journal.Last();
        Assert.Equal("POST", j.Method);
        Assert.Equal("messages.create_message", j.MatchedRoute);
    }

    [Fact]
    public async Task Messages_Create_Error()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var status = await AssertErrorAsync("messages.create_message", 500,
            () => tree.Messages.CreateAsync("x", "x"));
        Assert.Equal(500, status);
    }

    [Fact]
    public async Task Messages_Update_Success()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        ...
        Assert.Equal("messages.update_message", j.MatchedRoute);
    }

    [Fact]
    public async Task Messages_Update_Error()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
