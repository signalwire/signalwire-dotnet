...
/// <summary>Generated full-mock REST wire tests for the 'fax' namespace.</summary>
...
public class FaxGeneratedTest : CoverageBase
{
    public FaxGeneratedTest(MockServerFixture fixture) : base(fixture) { }

    private ResourceTree NewTree() => new(NewHttp());

    [Fact]
    public async Task LogsFax_Get_Success()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var body = await tree.Logs.Fax.GetAsync("x");
        Assert.NotNull(body);
        var j = Fixture.Harness.Journal.Last();
        Assert.Equal("GET", j.Method);
        Assert.Equal("fax.get_fax_log", j.MatchedRoute);
    }

    [Fact]
    public async Task LogsFax_Get_Error()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var status = await AssertErrorAsync("fax.get_fax_log", 500,
            () => tree.Logs.Fax.GetAsync("x"));
        Assert.Equal(500, status);
    }

    [Fact]
    public async Task LogsFax_List_Success()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var body = await tree.Logs.Fax.ListAsync();
        Assert.NotNull(body);
        var j = Fixture.Harness.Journal.Last();
        Assert.Equal("GET", j.Method);
        Assert.Equal("fax.list_fax_logs", j.MatchedRoute);
    }

    [Fact]
    public async Task LogsFax_List_Error()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var status = await AssertErrorAsync("fax.list_fax_logs", 500,
            () => tree.Logs.Fax.ListAsync());
        Assert.Equal(500, status);
    }
}
