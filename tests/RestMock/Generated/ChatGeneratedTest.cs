...
/// <summary>Generated full-mock REST wire tests for the 'chat' namespace.</summary>
[Trait("Category", "RestCoverage")]
public class ChatGeneratedTest : CoverageBase
{
    public ChatGeneratedTest(MockServerFixture fixture) : base(fixture) { }

    private ResourceTree NewTree() => new(NewHttp());

    [Fact]
    public async Task Chat_CreateToken_Success()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var body = await tree.Chat.CreateTokenAsync(1, new Dictionary<string, object?>());
        Assert.NotNull(body);
        var j = Fixture.Harness.Journal.Last();
        Assert.Equal("POST", j.Method);
        Assert.Equal("chat.create_chat_token", j.MatchedRoute);
    }

    [Fact]
    public async Task Chat_CreateToken_Error()
    {
        if (!Fixture.Available) return;
        var tree = NewTree();
        var status = await AssertErrorAsync("chat.create_chat_token", 500,
            () => tree.Chat.CreateTokenAsync(1, new Dictionary<string, object?>()));
