...
// Shared context for the registration/usage fragments below.
...
<!-- snippet: no-compile references-page-class (CrmLookupSkill is the third-party subclass defined earlier) -->
```csharp
// Register at startup
SkillRegistry.Instance.RegisterSkill("crm_lookup", () => new CrmLookupSkill());
```
...
1. **Registration** -- `SkillRegistry.Instance.RegisterSkill()` maps a name to a factory
