...
signalwire.core.skill_base.SkillBase.logger: approved: dotnet-no-public-logger — VERIFIED not a rename: dotnet's SkillBase exposes NO public logger member under any name (no Logger property, no GetLogger). Python's @property `logger` (class:get_logger) has no accessor twin here; logging is done via a private field. Not a language limit — pending API sign-off to add a public accessor.
