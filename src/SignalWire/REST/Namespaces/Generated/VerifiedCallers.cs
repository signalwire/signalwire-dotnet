// <auto-generated>
// Code generated by scripts/generate_rest.py; DO NOT EDIT.
//
// AUTO-GENERATED from porting-sdk/rest-apis/ (x-sdk-* markup) — regenerate with:
//   python3 scripts/generate_rest.py
//
// Generated REST resource for the 'relay-rest' namespace.
// </auto-generated>
#nullable enable

using System.Collections.Generic;
using System.Threading;
using System.Threading.Tasks;

namespace SignalWire.REST.Namespaces.Generated;

/// <summary>
/// VerifiedCallers — REST resource for the relay-rest API.
/// </summary>
public class VerifiedCallers : SignalWire.REST.CrudResource
{
    public VerifiedCallers(SignalWire.REST.HttpClient client)
        : base(client, "/api/relay/rest/verified_caller_ids")
    {
    }

    /// <summary>
    /// Generated from operation <c>redial_verification_call</c> (POST /verified_caller_ids/{id}/verification).
    /// </summary>
    /// <param name="requestOptions">Per-call request options (timeout/retries/abort) overriding the client defaults.</param>
    public Task<Dictionary<string, object?>> RedialVerificationAsync(string id, RequestOptions? requestOptions = null, CancellationToken cancellationToken = default)
    {
        return Client.PostAsync(Path(id, "verification"), null, requestOptions: requestOptions, cancellationToken: cancellationToken);
    }

    /// <summary>
    /// Generated from operation <c>validate_verification_code</c> (PUT /verified_caller_ids/{id}/verification).
    /// </summary>
    /// <param name="verificationCode">Wire field <c>verification_code</c>.</param>
    /// <param name="extras">Forward-compat body fields merged onto the request.</param>
    /// <param name="requestOptions">Per-call request options (timeout/retries/abort) overriding the client defaults.</param>
    public Task<Dictionary<string, object?>> SubmitVerificationAsync(string id, string verificationCode, Dictionary<string, object?>? extras = null, RequestOptions? requestOptions = null, CancellationToken cancellationToken = default)
    {
        var _reqBody = new Dictionary<string, object?>();
        _reqBody["verification_code"] = verificationCode;
        if (extras is not null)
        {
            foreach (var kv in extras)
            {
                _reqBody[kv.Key] = kv.Value;
            }
        }
        return Client.PutAsync(Path(id, "verification"), _reqBody, requestOptions: requestOptions, cancellationToken: cancellationToken);
    }
}
